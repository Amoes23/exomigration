function Test-OnPremMailboxAuditConfiguration {
    <#
    .SYNOPSIS
        Tests on-premises mailbox audit log configuration for migration readiness.
    
    .DESCRIPTION
        Checks if an on-premises mailbox has custom audit log settings that need to be preserved
    ...
    .EXAMPLE
        ...
        Test-OnPremMailboxAuditConfiguration -EmailAddress "user@contoso.com" -Results $results
    ...
    try {
        Write-Log -Message "Checking on-premises mailbox audit configuration for: $EmailAddress" -Level "INFO"
        ...
        if ($mailbox.AuditLogAgeLimit) {
            # Parse the timespan (comes as dd.hh:mm:ss format in on-premises)
