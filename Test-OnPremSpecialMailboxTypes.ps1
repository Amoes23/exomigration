function Test-OnPremSpecialMailboxTypes {
    <#
    .SYNOPSIS
        Tests if an on-premises mailbox is a special type that requires specific migration handling.
    
    .DESCRIPTION
        Checks if the on-premises mailbox is a special type such as shared, resource, discovery, 
    ...
    .EXAMPLE
        ...
        Test-OnPremSpecialMailboxTypes -EmailAddress "user@contoso.com" -Results $results
    ...
    try {
        Write-Log -Message "Checking for special on-premises mailbox type: $EmailAddress" -Level "INFO"
        ...
        # Check for Arbitration Mailbox role
        try {
            # Arbitration mailboxes have specific role assignments
            $mailboxRoles = Get-MailboxDatabase -Identity $mailbox.Database | Get-DatabaseAvailabilityGroup
            if ($mailboxRoles -and $mailboxRoles.ConfiguredFlags -contains "ArbitrationMailbox") {
                $Results.IsSpecialMailbox = $true
                $Results.SpecialMailboxType = "ArbitrationMailbox"
                $Results.SpecialMailboxGuidance = $specialTypes["ArbitrationMailbox"].Guidance
                
                $Results.Errors += $specialTypes["ArbitrationMailbox"].Warning
                $Results.ErrorCodes += "ERR026"
                
                Write-Log -Message "Arbitration mailbox detected for $EmailAddress" -Level "ERROR" -ErrorCode "ERR026"
                Write-Log -Message "  Guidance: $($specialTypes["ArbitrationMailbox"].Guidance)" -Level "INFO"
            }
        }
        catch {
            Write-Log -Message "Could not check for Arbitration mailbox role: $_" -Level "DEBUG"
        }
